...
def split_by_seconds(filename, split_length, vcodec="copy", acodec="copy",
    ...
    ouput = subprocess.Popen("ffmpeg -i "+filename, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
...
def split_by_chunks(filename, split_count, vcodec="copy", acodec="copy",
    ...
    if split_count and split_count <= 1:
        print("Split counts can't be 1 or less")
        raise SystemExit
...
if __name__ == '__main__':
    ...
    mode = int(input("請輸入要怎麼分割 1.用秒數分割 2.用份數分割 3.自定義檔案分割（json,csv） 4.自定義分割（輸入start_time,length）："))
    if mode == 1:
